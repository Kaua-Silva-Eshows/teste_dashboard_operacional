...
WHERE N5.VM_ACTIVE = '1'
  AND (DATE(CI.DATA_CONTAGEM) >= '{day}' AND DATE(CI.DATA_CONTAGEM) <= '{day2}' OR CI.DATA_CONTAGEM IS NULL)
...
LEFT JOIN (
  ...
  WHERE DATE(DR.COMPETENCIA) >= '2025-06-01' AND DATE(DR.COMPETENCIA) <= '2025-06-30'
  AND DRI.VALOR > 0
GROUP BY DRI.ID
) AS DRI ON DRI.FK_INSUMO = N5.ID
LEFT JOIN T_CONTAGEM_INSUMOS CI ON CI.FK_INSUMO = N5.ID
WHERE FTP.ID IN ('100', '101')
AND (DATE(CI.DATA_CONTAGEM) >= '{day}' 
AND DATE(CI.DATA_CONTAGEM) <= '{day2}' OR CI.DATA_CONTAGEM IS NULL)
GROUP BY DRI.FK_INSUMO
