...
def new_implementation():
    return get_dataframe_from_query ("""
WITH SHOWS_PADRAO_FILTRADO AS (
    SELECT FK_COMPANIES,
           DIA_DA_SEMANA,
           TIME(HORARIO_INICIO) AS HORA_INICIO,
           TIME(HORARIO_FIM) AS HORA_FIM,
           STATUS,
           ROW_NUMBER() OVER (
               PARTITION BY FK_COMPANIES, DIA_DA_SEMANA, TIME(HORARIO_INICIO), TIME(HORARIO_FIM)
               ORDER BY STATUS DESC
           ) AS RN
    FROM T_SHOWS_PADRAO
),
INATIVOS_POR_CASA AS (
    SELECT FK_COMPANIES,
           COUNT(*) AS QTD_INATIVOS
    FROM SHOWS_PADRAO_FILTRADO
    WHERE STATUS = 0
    GROUP BY FK_COMPANIES
),
SHOWS_ATIVOS_POR_CASA AS (
    SELECT FK_COMPANIES,
           COUNT(*) AS QTD_ATIVOS
    FROM SHOWS_PADRAO_FILTRADO
    WHERE STATUS = 1
...
STATUS_COMPANIES AS (
    SELECT NAME,
           MIN(CASE WHEN FK_STATUS_COMPANY = '100' THEN LOG_DATE END) AS LOG_100,
           MIN(CASE WHEN FK_STATUS_COMPANY = '102' THEN LOG_DATE END) AS LOG_102
    FROM ZLOG_T_COMPANIES
    GROUP BY NAME
)

SELECT DATE_FORMAT(C.CREATED_AT, '%d/%m/%Y') AS 'CREATED AT',
       SC.STATUS,
       COALESCE(GC.GRUPO_CLIENTES, '—') AS GRUPO,
       C.ID AS 'CASA ID',
       C.NAME AS 'CASA',
       COALESCE(OP.NOME, '—') AS 'OPERADOR',
       COALESCE(KA.NOME, '—') AS 'KEY ACCOUNT',
       C.CITY AS CIDADE,
       CASE WHEN C.CONTROLADORIA_ESHOWS = '00' THEN 'Não' ELSE 'Sim' END AS 'CONTROLADORIA',
       COALESCE(DATE_FORMAT(C.INICIO_DA_OPERACAO, '%d/%m/%Y'), '—') AS 'INÍCIO DA OPERAÇÃO',
       CASE
       ...
       CASE 
           WHEN SAPC.QTD_ATIVOS IS NULL THEN 'Não possui'
           WHEN COALESCE(IPC.QTD_INATIVOS, 0) = 0 THEN 'Sim'
           ELSE CONCAT(COALESCE(IPC.QTD_INATIVOS, 0), ' inativo(s)')
       END AS 'SHOW PADRÃO ATIVO?',
       CASE WHEN S100.LOG_100 IS NOT NULL THEN CONCAT(TIMESTAMPDIFF(DAY, S100.LOG_100, NOW()), ' dias') ELSE 'Não implantado' END AS 'TEMPO EM IMPLANTAÇÃO',
       CASE WHEN S102.LOG_102 IS NOT NULL THEN CONCAT(TIMESTAMPDIFF(DAY, S102.LOG_102, NOW()), ' dias') ELSE 'Não estabilizado' END AS 'TEMPO EM ESTABILIZAÇÃO'
FROM T_COMPANIES C
INNER JOIN T_STATUS_COMPANIES SC ON SC.ID = C.FK_STATUS_COMPANY
LEFT JOIN T_GRUPOS_DE_CLIENTES GC ON GC.ID = C.FK_GRUPO
LEFT JOIN T_KEYACCOUNT_ESTABELECIMENTO KA ON KA.ID = C.FK_KEYACCOUNT
LEFT JOIN STATUS_COMPANIES S100 ON C.NAME = S100.NAME
LEFT JOIN STATUS_COMPANIES S102 ON C.NAME = S102.NAME
LEFT JOIN T_PROPOSTAS P ON C.ID = P.FK_CONTRANTE
LEFT JOIN T_OPERADORES OP ON OP.ID = C.FK_OPERADOR
LEFT JOIN SHOWS_PADRAO_FILTRADO SPF ON SPF.FK_COMPANIES = C.ID
LEFT JOIN INATIVOS_POR_CASA IPC ON IPC.FK_COMPANIES = C.ID
LEFT JOIN SHOWS_ATIVOS_POR_CASA SAPC ON SAPC.FK_COMPANIES = C.ID
WHERE C.FK_STATUS_COMPANY IN (100, 102)
  AND C.ACTIVE = 1
  AND (
...
SELECT 
    ...
    C.NAME AS 'CASA',
    CASE 
    		WHEN C.CONTROLADORIA_ESHOWS = '0' THEN 'NÃO'
    		WHEN C.CONTROLADORIA_ESHOWS = '1' THEN 'SIM'
    END CONTROLADORIA,
...
@st.cache_data
def artist_cancelation(day1, day2, filters=''):
    return get_dataframe_from_query(f"""
SELECT
  A.ID,
  A.NOME AS 'ARTISTA',
  COUNT(P.ID) AS 'TOTAL DE SHOWS',
  SUM(CASE WHEN P.FK_STATUS_PROPOSTA = '102' THEN 1 ELSE 0 END) AS 'RECUSADA',
  SUM(CASE WHEN P.FK_STATUS_PROPOSTA IS NULL THEN 1 ELSE 0 END) AS 'CANCELAMENTO INTERNO',
  COALESCE((SUM(CASE WHEN P.FK_STATUS_PROPOSTA = '102' THEN 1 ELSE 0 END) / NULLIF(COUNT(P.ID) - SUM(CASE WHEN P.FK_STATUS_PROPOSTA IS NULL THEN 1 ELSE 0 END), 0)) * 100, 0) AS 'RECUSA %'
FROM T_PROPOSTAS P
LEFT JOIN T_PROPOSTA_STATUS PS ON PS.ID = P.FK_STATUS_PROPOSTA
...
LEFT JOIN T_MOTIVO_CANCELAMENTO_PROPOSTA MCP ON MCP.FK_PROPOSTA = P.ID
LEFT JOIN T_MOTIVO_CANCELAMENTO MC ON MC.ID = MCP.FK_ID_SOLICITACAO_CANCELAMENTO
WHERE P.DATA_INICIO >= STR_TO_DATE('{day1}', '%d/%m/%Y')
AND P.DATA_INICIO <= STR_TO_DATE('{day2}', '%d/%m/%Y')
{filters}
GROUP BY A.ID, A.NOME
HAVING SUM(CASE WHEN P.FK_STATUS_PROPOSTA = '102' THEN 1 ELSE 0 END) > 0
...
SELECT
    ...
    SUM(CASE WHEN P.FK_STATUS_PROPOSTA IS NULL THEN 1 ELSE 0 END) AS 'CANCELAMENTO INTERNO',
    COALESCE(
  (SUM(CASE WHEN P.FK_STATUS_PROPOSTA = '102' THEN 1 ELSE 0 END) / NULLIF(COUNT(P.ID) - SUM(CASE WHEN P.FK_STATUS_PROPOSTA IS NULL THEN 1 ELSE 0 END), 0)) * 100, 0) AS 'RECUSA %'
FROM T_PROPOSTAS P
LEFT JOIN T_PROPOSTA_STATUS PS ON PS.ID = P.FK_STATUS_PROPOSTA
LEFT JOIN T_COMPANIES C ON C.ID = P.FK_CONTRANTE
LEFT JOIN T_GRUPOS_DE_CLIENTES GC ON GC.ID = C.FK_GRUPO
WHERE 
    P.DATA_INICIO >= STR_TO_DATE('{day1}', '%d/%m/%Y')
    AND P.DATA_INICIO <= STR_TO_DATE('{day2}', '%d/%m/%Y')
GROUP BY C.ID, C.NAME, GC.NOME
HAVING SUM(CASE WHEN P.FK_STATUS_PROPOSTA = '102' THEN 1 ELSE 0 END) > 0
   OR SUM(CASE WHEN P.FK_STATUS_PROPOSTA IS NULL THEN 1 ELSE 0 END) > 0
ORDER BY COUNT(C.ID) DESC;
    """)
@st.cache_data
def artist_cancelation_detailed(day1, day2, id_artist):
    return get_dataframe_from_query(f"""
WITH MinLogDate AS (
    SELECT 
    ...
    WHERE ZP2.FK_STATUS_PROPOSTA = '102' OR ZP2.FK_STATUS_PROPOSTA IS NULL
    GROUP BY ZP2.ID
)
SELECT
    ...
    C.NAME AS 'CASA',
    A.NOME AS 'ARTISTA',
    CASE WHEN PS.DESCRICAO IS NULL THEN 'Cancelada' ELSE PS.DESCRICAO END 'STATUS',
    CASE 
    ...
    END AS 'DIFERENCA',
    COALESCE(MC.TEXTO_MOTIVO, MR.MOTIVO) AS 'MOTIVO',
    COALESCE(MCP.DESCRICAO_CANCELAMENTO, MRP.DESCRICAO_RECUSA) AS 'DESCRIÇÃO'
...
LEFT JOIN T_MOTIVO_CANCELAMENTO_PROPOSTA MCP ON MCP.FK_PROPOSTA = P.ID
LEFT JOIN T_MOTIVO_CANCELAMENTO MC ON MC.ID = MCP.FK_ID_SOLICITACAO_CANCELAMENTO
LEFT JOIN T_MOTIVO_RECUSA_PROPOSTA MRP ON MRP.ID = P.FK_MOTIVO_RECUSA
LEFT JOIN T_MOTIVO_RECUSA MR ON MR.ID = MRP.FK_MOTIVO_RECUSA
LEFT JOIN T_COMPANIES C ON C.ID = P.FK_CONTRANTE
LEFT JOIN MinLogDate MLD ON MLD.PROPOSTA_ID = P.ID
WHERE (P.FK_STATUS_PROPOSTA = '102' OR P.FK_STATUS_PROPOSTA IS NULL)
...
ORDER BY P.DATA_INICIO;
    """)
@st.cache_data
def companie_cancelation_detailed(day1, day2, filters):
    return get_dataframe_from_query(f"""
WITH MinLogDate AS (
    SELECT 
        ZP2.ID AS PROPOSTA_ID,
        MIN(ZP2.LOG_DATE) AS MIN_LOG_DATE,
        MIN(CASE 
            WHEN ZP2.FK_STATUS_PROPOSTA IS NULL THEN 'NULL'
            WHEN ZP2.FK_STATUS_PROPOSTA = '0' THEN '0'
            WHEN ZP2.FK_STATUS_PROPOSTA = '102' THEN '102'
        END) AS STATUS_REFERENCIA
    FROM ZLOG_T_PROPOSTAS ZP2
    WHERE ZP2.FK_STATUS_PROPOSTA IS NULL OR ZP2.FK_STATUS_PROPOSTA IN ('0', '102')
    GROUP BY ZP2.ID
)
SELECT
    DATE_FORMAT(P.DATA_INICIO, '%d/%m/%Y') AS 'DATA',
    ...
    C.NAME AS 'CASA',
    A.NOME AS 'ARTISTA',
    CASE WHEN P.FK_STATUS_PROPOSTA IS NULL OR P.FK_STATUS_PROPOSTA = '0' THEN 'Cancelada'	ELSE 'Recusada' END 'STATUS',
    CASE 
        WHEN DATEDIFF(P.DATA_INICIO, MLD.MIN_LOG_DATE) = 0 THEN 
            CONCAT(TIMESTAMPDIFF(HOUR, MLD.MIN_LOG_DATE, P.DATA_INICIO), ' horas')
        ELSE 
...
LEFT JOIN ZLOG_T_PROPOSTAS ZP ON ZP.ID = P.ID
LEFT JOIN MinLogDate MLD ON MLD.PROPOSTA_ID = P.ID
WHERE (P.FK_STATUS_PROPOSTA IS NULL OR P.FK_STATUS_PROPOSTA IN ('0', '102'))
AND P.DATA_INICIO BETWEEN STR_TO_DATE('{day1}', '%d/%m/%Y') AND STR_TO_DATE('{day2}', '%d/%m/%Y')
{filters}
GROUP BY P.ID;
    """)
