...
WITH PROPOSTAS_CLASSIFICADAS AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY CASE WHEN PS.DESCRICAO = 'Aceita' THEN 1 ELSE 2 END, P.DATA_INICIO
        ) AS RN
    FROM T_PROPOSTAS P
    LEFT JOIN T_PROPOSTA_STATUS PS ON PS.ID = P.FK_STATUS_PROPOSTA
),

-- Subconsulta para casas com pelo menos uma proposta com status 'Aceita'
PRIMEIRA_PROPOSTA_ACEITA AS (
    SELECT
        COMPANY_ID,
        MIN(DATA_INICIO) AS DATA_INICIO
    FROM PROPOSTAS_CLASSIFICADAS
    WHERE STATUS_DESCRICAO = 'Aceita'
    GROUP BY COMPANY_ID
),
...
PROPOSTAS_SEM_101 AS (
    ...
    LEFT JOIN PRIMEIRA_PROPOSTA_ACEITA PP
