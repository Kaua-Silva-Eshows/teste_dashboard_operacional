...
def biuldArtistCancelation(artistCancelation, companieCancelation, artistCancelationDetailed, companieCancelationDetailed):
    ...
    row1 = st.columns(2)
    with row1[0]:
        day_ArtistCancelation_data = Artist_Cancelation(day_Cancelation1.strftime('%d/%m/%Y'),day_Cancelation2.strftime('%d/%m/%Y'))
        filtered_copy_artist, count = component_plotDataframe(day_ArtistCancelation_data, "Artistas")
    ...
    with row1[1]:
        day_companieCancelation_data = Companie_Cancelation(day_Cancelation1.strftime('%d/%m/%Y'),day_Cancelation2.strftime('%d/%m/%Y'))
        filtered_copy_companie, count = component_plotDataframe(day_companieCancelation_data, "Casas")
