...
def buildShowlighthouse(showMonitoring, nextShows, showToCancel, churnCompanies, newCompanies): #housesImplementationStabilization , transfeeraStatementReport)
    ...
    tile.write(f"<p style='text-align: center;'>Total de artistas com show pela primeira vez</br>{quanty_0 + quanty_1}</p>", unsafe_allow_html=True)

    row6 = st.columns(1)

    with row6[0]: 
        with st.expander("Visualizar Artistas com shows pela Primeira vez"):
            filtered_copy, count = component_plotDataframe(artists_filtred, 'Tabela De Artistas com shows pela Primeira vez')
            st.session_state['filtered_copy'] = filtered_copy
    ...
    with tab1:
        ...
        filtered_copy, count = component_plotDataframe(nextShows, 'Shows na Proxima 1 Hora')
        function_copy_dataframe_as_tsv(filtered_copy)
        function_box_lenDf(len_df=count, df=filtered_copy, y='-100', x='500', box_id='box1')
    ...
    with tab2:
        ...
        filtered_copy, count = component_plotDataframe(filtredShowMonitoring, 'Monitoramento de shows')
        function_copy_dataframe_as_tsv(filtered_copy)
        function_box_lenDf(len_df=count, df=filtered_copy, y='-100', x='500', box_id='box1')
    ...
    with tab4:
        ...
        filtered_copy, count = component_plotDataframe(filtredShowMonitoring[filtredShowMonitoring['STATUS'] == 'Pendente'], 'Shows Com Status Pendente')
    ...
    with tab5:
        ...
        with row11[0]:
            ...
            filtered_copy, count = component_plotDataframe(churn_companies_data, "Sem Recorrência")
            function_copy_dataframe_as_tsv(filtered_copy)
            function_box_lenDf(len_df=count, df=filtered_copy,y='-130', x='440', box_id='box2')
        
        with row11[1]:
            new_companies_data = new_companies(day_churn_new.strftime('%Y-%m-%d'))
            filtered_copy, count = component_plotDataframe(new_companies_data, "Buscar Recorrência")
            function_copy_dataframe_as_tsv(filtered_copy)
            function_box_lenDf(len_df=count, df=filtered_copy,y='-130', x='440', box_id='box2')
